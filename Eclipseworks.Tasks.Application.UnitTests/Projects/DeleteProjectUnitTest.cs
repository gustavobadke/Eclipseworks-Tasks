...
using Eclipseworks.Tasks.Application.UseCases.Projects.Commands.DeleteProject;
    ...
    public class DeleteProjectUnitTest
    {
        ...
        private static Mock<ITaskRepository> GetTaskRepository(bool includePendingTask)
        {
            var tasks = new List<Domain.Entities.Task> {
                new Domain.Entities.Task { Id = Guid.NewGuid(), Status = Domain.Enums.StatusTask.Done },
            };

            if (includePendingTask)
                tasks.Add(new Domain.Entities.Task { Id = Guid.NewGuid(), Status = Domain.Enums.StatusTask.Todo });

            Mock<ITaskRepository> repoTaskMock = new(MockBehavior.Strict);
            repoTaskMock
                .Setup(s => s.ListAsync(It.IsAny<Guid>()))
                .Returns(System.Threading.Tasks.Task.FromResult(tasks as IEnumerable<Domain.Entities.Task>));
